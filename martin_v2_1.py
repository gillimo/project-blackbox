...
def run_command(command_str):
    try:
        if 'nano' in command_str or 'raspi-config' in command_str:
            ...
            process = Popen(command_str, shell=True, stdout=PIPE, stderr=PIPE, bufsize=1, universal_newlines=True)

            # Initialize a tqdm bar for real-time command output
            lines = []
            for line in tqdm(process.stdout, desc="Reading command output", unit="line"):
                lines.append(line)
                print(line.strip())
        ...
        if process.returncode == 0:
            ...
            return False, stderr.strip() if stderr else ""
